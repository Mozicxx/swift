...
class Enginex {
	 ...
	 */
	public function parseLiteral(string $data) {
		$this->literals = array();
	 ...
	 */
	public function restoreLiteral(string $data) {
		$pattern = '/<literal>(.*?)<\/literal>/i';
		return preg_replace_callback( $pattern, function ($matches) {
			$key = $matches [1];
			return isset( $this->literals [$key] ) ? '<literal>' . $this->literals [$key] . '</literal>' : $matches [0];
		}, $data );
	}
	...
	protected function parsePhp(string $data) {
		...
		return preg_replace_callback( $pattern, function ($matches) {
			$this->phps [] = $matches [1];
			$key = count( $this->phps ) - 1;
			return '<php>' . ( string ) $key . '</php>';
		}, $data );
	}
	...
	protected function restorePhp(string $data) {
		...
		return preg_replace_callback( $pattern, function ($matches) {
			$key = $matches [1];
			return isset( $this->phps [$key] ) ? '<?php ' . $this->phps [$key] . ' ?>' : $matches [0];
		}, $data );
	}
 ...
 * ********************************************************************************************************************************
 */
$ee = new Enginex( 'a.b.c' );
$data = 'I<php>aaa</php>,love your<php>bbb</php>';
echo $format = $ee->parsePhp( $data );
echo '=====';
echo $out = $ee->restorePhp( $format );
